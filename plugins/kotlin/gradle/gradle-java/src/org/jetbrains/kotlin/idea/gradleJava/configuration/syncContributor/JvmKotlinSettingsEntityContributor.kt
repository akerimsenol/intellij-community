...
import org.jetbrains.kotlin.idea.facet.getAdditionalArgumentsString
...
class JvmKotlinSettingsEntityContributor: GradleSourceRootSyncContributorExtension {
    override suspend fun configureSourceSetModules(
        ...
        configureJvmKotlinSettingsEntity(context, projectModel, moduleEntity, sourceSet.name)
    }
}
...
fun configureJvmKotlinSettingsEntity(
    ...
    val pluginVersion = kotlinGradleModel.kotlinGradlePluginVersion?.versionString?.let {
    ...
    val jvmArguments = K2JVMCompilerArguments().apply {
        ...
        if (pluginOptions == null)
            pluginOptions = emptyArray()
        if (pluginClasspaths == null)
            pluginClasspaths = emptyArray()
    }
    val emptyArgs = K2JVMCompilerArguments::class.java.getDeclaredConstructor().newInstance()

    moduleEntity.kotlinSettings += createEmptyKotlinSettingsEntity(moduleEntity) {
        with(KotlinFacetSettingsWorkspaceModel(this)) {
            this.compilerSettings = CompilerSettings().apply {
                additionalArguments = getAdditionalArgumentsString(jvmArguments, emptyArgs)
            }
            this.targetPlatform = JvmIdePlatformKind.defaultPlatform
            this.compilerArguments = jvmArguments

            // default to plugin version for api/language version if not present
            this.apiLevel = this.apiLevel ?: pluginVersion
            this.languageLevel = this.languageLevel ?: pluginVersion
        }
    }
}
